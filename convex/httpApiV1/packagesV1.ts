...
const PACKAGE_PUBLISH_FORM_FIELDS = new Set(["payload", "files[]", "tarball"]);
...
function getFileParts(form: FormData, field: string, stringPartError: string) {
...
async function parseMultipartPackagePublish(
  ...
  const tarballFiles = getFileParts(form, "tarball", "Package tarball upload must be a file");
  if (tarballFiles.length > 1) throw new Error("Upload one package tarball");
  const tarballEntry = tarballFiles[0];
  const fileParts = getFileParts(form, "files[]", "Package publish files[] uploads must be files");
